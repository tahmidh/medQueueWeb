...
$get_doctor_list=mysql_query("SELECT pictures.path,user_details.login_id, user_details.address,user_details.first_name,user_details.last_name,doc_qualifications.shortbio FROM pictures,doc_qualifications,user_details WHERE login_id in (SELECT uid FROM user_login WHERE u_type='doctor') AND user_details.login_id = doc_qualifications.user_id AND user_details.login_id = pictures.u_id;");
  ...
  <div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12">
      ...
      while($row=mysql_fetch_array($get_doctor_list)){
        ...
        <p>' . $row['shortbio']. '</p>
        <p>' . $row['address']. '</p>
        <a href="docpro.php?src=php&id=' . $row['login_id']. '" class="btn btn-default">View Details</a> </div>
        </div>
        </div>
        </div> 
        ';
