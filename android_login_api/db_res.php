...
function res(doc_id, minit, talbe0) 
    {
        ...
        while(i<tablee.length){
            ...
            var msg= "Your Appointment with Dr. "+ docName +" has been rescheduled to "+ newstrt +"(previous time: " + prstrt +")";
            console.log(msg);
            //send_SMS(msg, number);
            $.ajax({ method: "POST", url: "updateDelay.php",data: {
...
$min = 0;
$docName=0;
if (isset($_GET['id']) && isset($_GET['minute']) && isset($_GET['docName'])){
	$user_id=$_GET['id'];
	$min=$_GET['minute'];
...
<input type="text"  value="<?php echo htmlspecialchars($docName); ?>" id="dname" disabled/>
</table>
</body>
</html>